/**
 * Enrichments MFE - Effects
 *
 * Effects for enrichment domain operations (enrichments, comments, git ops log).
...
export function initEnrichmentEffects(_dispatch: AppDispatch): void {
  ...
  eventBus.on('wiki/gitOpsLog/load', async ({ limit }) => {
    try {
      const service = apiRegistry.getService(EnrichmentsApiService);
      const response = await service.getGitOpsLog(limit);
      eventBus.emit('wiki/gitOpsLog/loaded', { entries: response.entries || [] });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to load git ops log';
  ...
  eventBus.on('wiki/gitOpsLog/clear', async () => {
    try {
      const service = apiRegistry.getService(EnrichmentsApiService);
      await service.clearGitOpsLog();
      eventBus.emit('wiki/gitOpsLog/loaded', { entries: [] });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to clear git ops log';
      eventBus.emit('wiki/gitOpsLog/error', { error: message });
    }
  });
}
